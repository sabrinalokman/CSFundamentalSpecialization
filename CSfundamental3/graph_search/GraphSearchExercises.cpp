...
int GridGraph::countEdges() const
{
  ...
  {
    const auto &v1 = keyval.first;
    const auto &v1_neighbours = keyval.second;

    if (!v1_neighbours.empty())
    {
      for (const auto &v2 : v1_neighbours)
      {
        IntPairPair edge;
        if (v1 < v2)
        {
...
std::list<IntPair> graphBFS(const IntPair &start, const IntPair &goal, const GridGraph &graph)
{
  ...
  while (!exploreQ.empty() && !foundGoal && !tooManySteps)
  {
    ...
    for (auto neighbor : neighbors)
    {
      ...
      bool neighborWasAlreadyVisited = visitedSet.count(neighbor) == 1 ? true : false; // Change this...
      // ==================================================================
